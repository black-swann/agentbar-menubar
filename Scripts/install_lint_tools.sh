...
install_zip_binary() {
  ...
  if [[ -f "${tmp_dir}/${archive_binary_name}" ]]; then
    extracted_path="${tmp_dir}/${archive_binary_name}"
  else
    extracted_path="$(find "$tmp_dir" -type f -name "$archive_binary_name" | head -n 1 || true)"
  fi

  if [[ -z "$extracted_path" || ! -f "$extracted_path" ]]; then
    rm -f "$tmp_zip"
    rm -rf "$tmp_dir"
    fail "${label} binary '${archive_binary_name}' not found in archive"
  fi
...
case "$OS" in
  ...
  Linux)
    ...
    # Linux installs currently skip SHA256 enforcement.
    # If we later run lint in CI here, add pinned SHAs as well.
    log "WARN: Linux SHA256 verification not configured for ${ARCH}; installing anyway."
    install_zip_binary "SwiftFormat ${SWIFTFORMAT_VERSION}" "$SWIFTFORMAT_URL" "" "swiftformat" "swiftformat_linux"
