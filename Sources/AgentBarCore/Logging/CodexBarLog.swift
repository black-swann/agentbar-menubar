...
public enum AgentBarLog {
    ...
    public static func logger(_ category: String) -> AgentBarLogger {
        let logger = Logger(label: "\(AppIdentity.osLogSubsystem).\(category)")
        return AgentBarLogger { level, message, metadata in
...
public struct AgentBarLogger: Sendable {
    ...
    fileprivate init(_ logFn: @escaping @Sendable (AgentBarLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
