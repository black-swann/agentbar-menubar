    ...
    static func runUsage(_ values: ParsedValues) async {
        ...
        if let parsedSourceMode {
            ...
            if requiresWeb {
                Self.exit(
                    code: .failure,
                    message: "Error: selected source requires unsupported web dashboard support in this Linux build.",
                    output: output,
                    kind: .runtime)
            }
        }
        ...
        for p in providerList {
            ...
            // CLI usage should stay non-interactive.
        ...
        command: UsageCommandContext) -> UsageCommandOutput
    {
        ...
        let error = NSError(
            ...
            userInfo: [NSLocalizedDescriptionKey:
                "Error: selected source requires unsupported web dashboard support in this Linux build."])
        output.exitCode = .failure
        if command.format == .json {
            output.payload.append(Self.makeProviderErrorPayload(
                provider: provider,
